(() => {
  ...
  app.metrics.countClaimedNeighbors4 = (claimedSet, x, z) => {
    let count = 0;
  ...
  app.metrics.countEndpoints4 = (claimedSet) => {
    let count = 0;
    for (const k0 of claimedSet) {
      const { x, z } = parseKey(k0);
      const deg = app.metrics.countClaimedNeighbors4(claimedSet, x, z);
      if (deg === 1) count++;
    }
    return count;
  };

  app.metrics.perimeterEdges = (claimedSet) => {
    let p = 0;
    for (const k0 of claimedSet) {
      const { x, z } = parseKey(k0);
      for (const n of neighbors4(x, z)) {
  ...
  app.metrics.approxDiameter4 = (claimedSet) => {
  ...
  app.metrics.computeBounds = (claimedSet) => {
  ...
  app.metrics.updateBoundsWithCell = (bounds, x, z) => {
    if (!bounds) {
  ...
  app.metrics.maxFilledSquareSide = (claimedSet, bounds) => {
